...
public class HardActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        level1Button = findViewById(R.id.hard_lvl_1);
        level1Button.setOnClickListener(v -> startGameActivity(1));

        level2Button = findViewById(R.id.hard_lvl_2);
        level2Button.setOnClickListener(v -> startGameActivity(2));

        level3Button = findViewById(R.id.hard_lvl_3);
    ...
    private void updateButtonColors() {
        ...
        if (sharedPreferences.getBoolean("hard_level_1_completed", false)) {
            level1Button.setBackgroundColor(Color.GREEN);
        }
        if (sharedPreferences.getBoolean("hard_level_2_completed", false)) {
            level2Button.setBackgroundColor(Color.GREEN);
        }
        if (sharedPreferences.getBoolean("hard_level_3_completed", false)) {
